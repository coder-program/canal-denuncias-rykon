...
doc.fontSize(60).fillColor('#FF0000', 0.1).text('CONFIDENCIAL', 0, 300, { align: 'center' });
...
const downloadReport = async () => {
  ...
  try {
    // 1. Gerar dossiê
    const { data } = await apiClient.post(`/dossiers/complaint/${id}/generate`, { format: 'PDF' });

    // 2. Obter URL de download
    const downloadResponse = await apiClient.get(`/dossiers/${data.id}/download/pdf`);
